...
export async function createCompletionWorkaround(image: string, task: string) {

      const messages: ChatCompletionMessageParam[] = [
    {
      role: "system",
      content:
        `You are given a text interface to a web browser. You can use the following commands to navigate the web page in the image:
        - clickElementByTag: Clicks the first element with the given tag name. Call it like this -> clickElementByTag:{TAG_NAME}
        - logAnser: Logs the answer to the console. Call it like this -> logAnswer:{ANSWER}
        `,
    },
    {
      role: "user",
...
export function navigateWithClick(tag: string) {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (!tabs[0].id) return "Something went wrong.";
    chrome.tabs.sendMessage(
      tabs[0].id,
      {
        action: "clickElementByTag",
        uniqueTag: tag,
      },
      (response) => {
